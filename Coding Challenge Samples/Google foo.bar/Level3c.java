 ...
 */
public class Level3cAttempt2 {

    // careful of static when querying multiple times
    ...
    // Helper determines all possible arrangements while tabulating
    private static void arranger(int bricks, int prevStep) {
        if (bricks % 2 == 0) {
            int rightStep = (bricks / 2) - 1;
            for (int step = rightStep; step > prevStep; step--) {
                arrangements += 1;
                arranger(bricks - step, step);
            }
        } else {
            int rightStep = bricks / 2;
            for (int step = rightStep; step > prevStep; step--) {
                arrangements += 1;
                arranger(bricks - step, step);
            }
